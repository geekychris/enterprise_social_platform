...
final class AuthService {
    ...
    init() {
        ...
        if let token = UserDefaults.standard.string(forKey: "auth_token") {
            ...
            if let savedTenant = UserDefaults.standard.string(forKey: "auth_tenantId") {
    ...
    func login(username: String, password: String) async throws {
        ...
        let savedTenantId = api.tenantId
        await MainActor.run {
            ...
            self.tenantId = savedTenantId
            self.isAuthenticated = true
            self.debugMode = false
            persist(token: response.token, userId: uid, username: response.username, isAdmin: response.admin, tenantId: savedTenantId)
        }
    }
    ...
    func logout() {
        ...
        UserDefaults.standard.removeObject(forKey: "auth_tenantId")
    ...
    private func persist(token: String, userId: Int64, username: String, isAdmin: Bool, tenantId: String? = nil) {
        ...
        if let tenantId {
            UserDefaults.standard.set(tenantId, forKey: "auth_tenantId")
        }
    }
}
